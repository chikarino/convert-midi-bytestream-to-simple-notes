# MIDITONES Score Converter | MIDITONES 樂譜轉換器

Convert [MIDITONES](https://github.com/LenShustek/miditones)-generated bytestream into a simple Note array for embedded systems.

將 [MIDITONES](https://github.com/LenShustek/miditones) 生成的 bytestream 轉換為簡單的音符陣列，適用於嵌入式系統。

## Overview | 概述

This tool converts MIDITONES binary bytestream (stored in C `score[]` arrays) into a human-readable and easily-usable `Note` structure array containing frequency (Hz) and duration (frames).

此工具將 MIDITONES 二進制 bytestream（儲存在 C 語言的 `score[]` 陣列中）轉換為易讀且易用的 `Note` 結構陣列，包含頻率（Hz）和持續時間（幀數）。

## Features | 功能特點

- ✅ Parse MIDITONES bytestream format | 解析 MIDITONES bytestream 格式
- ✅ Support 'Pt' header detection | 支援 'Pt' 標頭偵測
- ✅ Convert MIDI notes to Hz frequencies | 將 MIDI 音符轉換為 Hz 頻率
- ✅ Frame-based timing (configurable FPS) | 基於幀數的時間（可配置 FPS）
- ✅ Automatic input file detection | 自動偵測輸入檔案

## Requirements | 系統需求

- Python 3.7 or higher | Python 3.7 或更高版本
- No external dependencies | 無需外部依賴

## Installation | 安裝

```bash
git clone https://github.com/yourusername/miditones-converter.git
cd miditones-converter
```

## Usage | 使用方式

### Basic Usage | 基本用法

```bash
# Auto-detect score.c and output to song.c
# 自動偵測 score.c 並輸出到 song.c
python convert.py

# Specify input and output files
# 指定輸入和輸出檔案
python convert.py -i my_score.c -o my_song.c
```

### Command Line Options | 命令列選項

| Option | Description | 說明 |
|--------|-------------|------|
| `-i`, `--input` | Input .c file (default: auto-find score.c) | 輸入 .c 檔（預設自動尋找 score.c） |
| `-o`, `--output` | Output .c filename (default: song.c) | 輸出 .c 檔名（預設 song.c） |
| `--tone` | Tone generator to extract (default: 0) | 要抽取的 tone generator（預設 0） |
| `--fps` | Output frame rate (default: 60) | 輸出幀率（預設 60） |

### Examples | 範例

```bash
# Convert with custom frame rate
# 使用自定義幀率轉換
python convert.py --fps 30

# Extract specific tone generator
# 抽取特定的 tone generator
python convert.py --tone 1
```

## Workflow | 工作流程

```
┌─────────────┐     ┌───────────────┐     ┌─────────────┐     ┌─────────────┐
│  MIDI File  │ ──▶ │   MIDITONES   │ ──▶ │  score.c    │ ──▶ │   song.c    │
│  (.mid)     │     │   Compiler    │     │ (bytestream)│     │ (Note array)│
└─────────────┘     └───────────────┘     └─────────────┘     └─────────────┘
                                                    │
                                                    ▼
                                          This tool | 本工具
```

## Output Format | 輸出格式

The converter generates a C file with the following structure:

轉換器生成以下結構的 C 檔案：

```c
typedef struct { uint16_t frequency; uint16_t duration; } Note;

const Note song[] = {
    { 294, 468 },  // D4, ~7.800s
    { 262, 6 },    // C4, ~0.100s
    { 0, 30 },     // Rest | 休止符 ~0.500s
    // ...
};
const unsigned song_len = sizeof(song)/sizeof(song[0]);
```

## MIDITONES Bytestream Format | MIDITONES Bytestream 格式

| Byte | Description | 說明 |
|------|-------------|------|
| `'P','t'` | Header signature | 標頭識別碼 |
| `0x9t nn` | Note on (tone t, note nn) | 音符開始 |
| `0x8t` | Note off (tone t) | 音符結束 |
| `0xCt ii` | Program change | 樂器變更 |
| `0xF0` | End of score | 樂譜結束 |
| `bb bb` | Delay (15-bit ms, big-endian) | 延遲時間 |

## Use Cases | 應用場景

- 🎵 **Embedded Music Players** - Play music on microcontrollers (Arduino, STM32, etc.)
  
  **嵌入式音樂播放器** - 在微控制器上播放音樂（Arduino、STM32 等）

- 🔔 **Buzzer/Speaker Drivers** - Drive piezo buzzers or small speakers
  
  **蜂鳴器/揚聲器驅動** - 驅動壓電蜂鳴器或小型揚聲器

- 🎮 **Game Sound Effects** - Add music to embedded games
  
  **遊戲音效** - 為嵌入式遊戲添加音樂

## Related Projects | 相關專案

- [MIDITONES](https://github.com/LenShustek/miditones) - Original MIDI to bytestream converter

## Acknowledgments | 致謝

- Thanks to [Len Shustek](https://github.com/LenShustek) for creating MIDITONES
- 感謝 [Len Shustek](https://github.com/LenShustek) 創建 MIDITONES
